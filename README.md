...
2. `install_callback`：
   ...
   - **v0.6.33-24 起：Agent 的 browser tools + TUI 依赖预装进 FPK（`app/hermes-agent-node/`）**。安装时直接复制并跳过后台 `npm install`（官方 installer 默认 `NODE_DEPS_TIMEOUT=600s`，慢网会卡很久），首启大幅提速。
...
bash scripts/build-fpk.sh dist    # 自动探测并用 fnpack 构建，同时复制为 dist/fnos-hermes-studio_v0.6.33-24.fpk
```
...
# 产物：dist/fnos-hermes-studio_v0.6.33-24.fpk
```
...
2. 选择生成的 `fnos-hermes-studio_v0.6.33-24.fpk`，确认安装。
